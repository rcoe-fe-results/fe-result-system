...
const State = (() => {
  ...
  async function addSession(name, semester, batchYear) {
    const user = Auth.getUser();
    const session = {
      id:        'SES-' + Date.now(),
      name, semester: Number(semester), batchYear,
      status:    'Active',
  ...
  // For a given session + semester + branch, which students are KT-eligible?
  ...
  function getKTEligibleStudents(semester, branch) {
    ...
    for (const student of allBranchStudents) {
      ...
      const ktSubjects = ktRows.map(r => {
        // Find subject config from either semester
        const s1 = SEM1_SUBJECTS.find(s => s.code === r.subjectCode);
        const s2 = getSem2Subjects(student.branch).find(s => s.code === r.subjectCode);
        return s1 || s2 || { code: r.subjectCode, name: r.subjectName, marks:{}, credits:0 };
      }).filter(Boolean);
  ...
  async function submitEntries(session, entries) {
    ...
    for (const entry of entries) {
      ...
      const subjects = getSubjectsForSem(semester, student.branch);
